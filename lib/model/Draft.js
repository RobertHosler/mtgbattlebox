...
module.exports = (function() {
    ...
    var createDraft = function(draftId, cube, poolSize) {
        var shuffledCube = cube.slice();//make a copy of the cube
        MtgUtil.shuffle(shuffledCube);//shuffle it
        var draft = {};
        draft.id = draftId;
        draft.poolSize = poolSize;
        draft.activePlayer = Math.floor((Math.random() * 2) + 1);//The maximum is exclusive and the minimum is inclusive
        draft.pool = shuffledCube.slice(0, draft.poolSize);//TODO: check if poolSize is larger than cube
        draft.playerPools = ['', [], []];
        draft.playerSecret = ['', createPlayerSecret(), createPlayerSecret()];
        draft.turn = 1;
        draft.complete = false;
    }
    
    var createPlayerSecret = function() {
        var playerSecret = {};
        playerSecret.deck = [];
        playerSecret.sideboard = [];
        playerSecret.pool = [];
    }
